package nwt.kts.backend.exceptions.handler;
...
public class RestExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleDataIntegrityViolation(DataIntegrityViolationException ex,
        ...
        if (ex.getCause() instanceof ConstraintViolationException) {
            ...
            if ("23505".matches(cve.getSQLState())) {
                String[] strs = ex.getMostSpecificCause().getLocalizedMessage().split("=");
                String[] strs1 = strs[0].split("Key");
                String reason = strs1[1].trim().replace("(", "").replace(")", "");
                return buildResponseEntity(new ApiError(HttpStatus.CONFLICT, "Selected " + reason + " is already taken. Please choose another one.", ex.getCause()));
            }
